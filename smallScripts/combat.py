...
def design_mat(mod, numerical_covariates, batch_levels):
    ...
    for col in factor_matrix.columns:
        design_matrix = patsy.dmatrix(
            "~ 0 + C({}, levels={})".format(col, list(set(factor_matrix[col]))),
            factor_matrix, return_type="dataframe"
        )
        design = design.join(design_matrix)
...
def combat(data, batch, model=None, numerical_covariates=None):
    ...
    """
    if type(data) is str:
        data = pd.read_csv(data, sep=None, header=0, index_col=0, engine='python')
    if type(batch) is str:
        batch = pd.read_csv(batch, sep=None, header=0, index_col=0, engine='python')
    batch = batch.loc[data.columns]
    if type(model) is str:
        model = pd.read_csv(model, sep=None, header=0, index_col=0, engine='python')
    ...
    bayesdata.to_csv('batch.corrected.data', index=True, header=True, sep='\t')
    return bayesdata
...
if __name__ == "__main__":
    ...
    xcmds.xcmds(locals(), include=['combat'])

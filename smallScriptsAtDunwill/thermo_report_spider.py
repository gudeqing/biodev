...
def main(url='http://10.62.2.15/report/146/', login_url='http://10.62.2.15/login/'):
    ...
    final_overall_match_dict = OrderedDict([
        ...
        ('Clonal', '{:.2%}'.format(1- int(final_overall['Filtered: Polyclonal'])/int(final_overall['Library ISPs']))),
        ...
        ('AlignedReads (%)', '{:.2%}'.format(int(final_overall['Aligned Reads'])/int(final_overall['Total Reads']))),
        ('UnalignedReads (%)', '{:.2%}'.format(1-int(final_overall['Aligned Reads'])/int(final_overall['Total Reads']))),
    ...
    for x, y in sample_detail_dict.items():
        y = OrderedDict([
            ...
            ('Bases', y["total_bases"]),
            ('>=Q20 Bases (%)', '{:.2%}'.format(int(y['Q20_bases'])/int(y["total_bases"]))),
            ('Reads', y['read_count']),
            ('Mean Read Length (bp)', y["mean_read_length"]),
            ('Mapped Reads', y['mapped_reads']),
            ('Mapped Reads (%)', '{:.2%}'.format(int(y['mapped_reads'])/int(y['read_count']))),

#!/nfs2/software/miniconda2/bin/python
...
def run_fastq_version():
    ...
    if "NCM_HOME" in os.environ.keys():
        ...
        print("WARNNING : NCM_HOME is not defined yet. "
              "Therefore, program will try to search ngscheckmate_fastq file from the current directory")
        INSTALL_DIR = "./"
        ...
        #            print AUCs
            ...
            for i in range(0, len(samples)):
                ...
                if result[1] == 1:
                    ...
                    if out_tag == "stdout":
                        print(str(temp[i][0][:-4]) + '\tmatched\t',
                              str(temp[i][1][:-4]), '\t', round(samples[i], 4), '\t', round(depth, 2))
                    else:
                        out_f.write(str(temp[i][0][:-4]) + '\tmatched\t' + str(temp[i][1][:-4]) + '\t' + str(
                            round(samples[i], 4)) + '\t' + str(round(depth, 2)) + '\n')
                        out_matched.write(str(temp[i][0][:-4]) + '\tmatched\t' + str(temp[i][1][:-4]) + '\t' + str(
                            round(samples[i], 4)) + '\t' + str(round(depth, 2)) + '\n')
                else:
                    if out_tag == "stdout":
                        print(str(temp[i][0][:-4]) + '\tunmatched\t', str(temp[i][1][:-4]), '\t', round(samples[i],
                                                                                                  4), '\t', round(depth,
                                                                                                                  2))
                    else:
                        out_f.write(str(temp[i][0][:-4]) + '\tunmatched\t' + str(temp[i][1][:-4]) + '\t' + str(
                            round(samples[i], 4)) + '\t' + str(round(depth, 2)) + '\n')
    ...
    #            print AUCs
        ...
        for i in range(0, len(samples)):
            ...
            if result[1] == 1:
                ...
                if out_tag == "stdout":
                    print
                    str(temp[i][0][:-4]) + '\tmatched\t', str(temp[i][1][:-4]), '\t', round(samples[i], 4), '\t', round(
                        depth, 2)
                else:
                    out_f.write(str(temp[i][0][:-4]) + '\tmatched\t' + str(temp[i][1][:-4]) + '\t' + str(
                        round(samples[i], 4)) + '\t' + str(round(depth, 2)) + '\n')
                ...
                if out_tag == "stdout":
                    print(str(temp[i][0][:-4]) + '\tunmatched\t',
                          str(temp[i][1][:-4]), '\t', round(samples[i], 4), '\t', round(depth, 2))
                else:
                    out_f.write(str(temp[i][0][:-4]) + '\tunmatched\t' + str(temp[i][1][:-4]) + '\t' + str(
                        round(samples[i], 4)) + '\t' + str(round(depth, 2)) + '\n')
...
def generate_R_scripts():
    ...
    if len(feature_list) == 0:
        ...
        if len(feature_list) < 5:
            ...
            cmd = cmd + "pdf(\"" + outdir + "/" + pdf_tag + ".pdf\", width=" + str(
                math.log10(len(feature_list)) * 10) + ", height=7)\n"
...
def output_filter():
    ...
    with open(outdir + "/" + out_tag + "_all.txt", "r") as F:
        for line in F.readlines():
            ...
            if match == "matched":
                if sample1[sample1.index("TCGA"):sample1.index("TCGA") + 12] == sample2[
                                                                                sample2.index("TCGA"):sample2.index(
                                                                                        "TCGA") + 12]:
...
if __name__ == '__main__':
    ...
    Usage : python ncm_fastq.py -l INPUT_LIST_FILE -pt PT_FILE -O OUTPUT_DIR [options]
        ...
        Options
          ...
          -I STR        An input directory, if set, Using existed result from this directory. Added by gdq
            """
    ...
    parser.add_argument('-f', '--family_cutoff', dest='family_cutoff', action='store_true',
                        help='apply strict correlation threshold to remove family cases')
    parser.add_argument('-pt', '--pt', metavar='feature pattern file', required=True, dest='bed_file', action='store',
                        help='pattern file')
    parser.add_argument('-s', '--ss', metavar='subsampling_rate', dest='sub_rate', action='store',
                        help='subsampling rate (default 1.0)')
    parser.add_argument('-d', '--depth', metavar='desired_depth', dest='desired_depth', action='store',
                        help='as an alternative to a user-defined subsampling rate, let the program compute the subsampling rate given a user-defined desired_depth and the data')
    parser.add_argument('-R', '--reference_length', metavar='reference_length', dest='reference_length', action='store',
                        help="The reference length (default : 3E9) to be used for computing subsampling rate.")
    parser.add_argument('-L', '--pattern_length', metavar='pattern_length', dest='pattern_length', action='store',
                        help='The length of the flanking sequences being used to identify SNV sites. Default is 21bp.\nIt is recommended not to change this value, unless you have created your own pattern file with a different pattern length.')
    parser.add_argument('-p', '--maxthread', metavar='maxthread', dest='maxthread', action='store',
                        help='number of threads to use (default : 1 )')
    parser.add_argument('-j', '--nodeptherror', metavar='nodeptherror', dest='nodeptherror', action='store',
                        help='in case estimated subsampling rate is larger than 1, do not stop but reset it to 1 and continue')

    parser.add_argument('-O', '--outdir', metavar='output_dir', dest='outdir', action='store',
                        help='directory name for temp and output files')
    parser.add_argument('-N', '--outfilename', metavar='output_filename', dest='outfilename', action='store',
                        default="output", help='OutputFileName ( default : output ), -N filename')
    parser.add_argument('-l', '--list', metavar='input_file_list', required=False, dest='inputfilename', action='store',
                        help='Inputfile name that contains fastq file names, -I filename')
    parser.add_argument('-nz', '--nonzero', dest='nonzero_read', action='store_true',
                        help='Use non-zero mean depth of target loci as reference correlation. (default: Use mean depth of all target loci)')

    parser.add_argument('-t', '--testsamplename', metavar='test_samplename', dest='testsamplename', action='store',
                        help='file including test sample namses  with ":" delimeter (default : all combinations of samples), -t filename.\n-t option is for the previous NGSCheckMate version. No longer used.')
    parser.add_argument('-I', default=None, help="An input directory, if set, Using existed result from this directory")

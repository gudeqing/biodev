...
def enrichment_bar(xdata, ydata, category, stat_value, stat_cutoff=[0.001, 0.01, 0.05], cmap='PiYG',
                   fig_name='enrichment_bar', fig_size=(10, 6),
...
def hypergeom_test(data, sort_fdr=True, correct_method=3):
    ...
    result = zip(study_number_list, path_names, databases, classes, ratio_in_study_list, ratio_in_pop_list,
                 p_value_list, q_value_list, hit_genes, hit_links, typeIIs, typeIs)
    if sort_fdr:
...
if __name__ == '__main__':
    ...
    for deg_file in deg_files:
        ...
        f.write(
            '#Study_num\tTerm\tDatabase\tID\tRatio_in_study\tRatio_in_pop\tP-Value\tCorrected P-Value\tGenes\tHyperlink\ttypeII\ttypeI\n')
        for each in result:

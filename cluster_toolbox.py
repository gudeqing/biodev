...
def r_hcluster(exp_pd, transpose=False, n_clusters=10, method='average',
               metric='euclidean', output=None, prefix='', do_scale=True):
    """
        'fastcluster' was used, http://www.danifold.net/fastcluster.html?section=3.
        scipy.cluster.hierarchy.linkage could also do the same thing but slower.
        However, the documentation of scipy.cluster.hierarchy.linkage is pretty good.
...
def exp_cluster(exp_pd, n_clusters=10, s_cluster=True, g_cluster=True,
                s_cluster_type='hierarchy', g_cluster_type='hierarchy',
                s_method="complete", g_method="average",
                s_metric="correlation", g_metric="euclidean", use_r=True,
                output=None):
    result = []
    if use_r:
        hcluster = r_hcluster
    if s_cluster:
        ...
        if s_cluster_type == 'hierarchy':
            s_cluster_result = hcluster(exp_pd, transpose=True, n_clusters=n_clusters, method=s_method,
                                        metric=s_metric, output=output, prefix="sample.")
        else:
            s_cluster_result = kmeans(exp_pd, n_clusters=n_clusters, transpose=True, output=output, prefix='sample.')
        result.append(s_cluster_result)

    if g_cluster:
        # for gene
        if g_cluster_type == 'hierarchy':
            g_cluster_result = hcluster(exp_pd, transpose=False, n_clusters=n_clusters, method=g_method,
                                        metric=g_metric, output=output, prefix="seq.")
        else:
            g_cluster_result = kmeans(exp_pd, n_clusters=n_clusters, transpose=False, output=output, prefix='seq.')
    ...
    exp_pd.to_csv(out_dir, sep='\t', header=True, index=True)
    return result
...
if __name__ == '__main__':
    ...
    if args.group:
        ...
        group_dict = None
    #
    exp_pd = process_exp_matrix(args.exp, log_base=args.log_base, group_dict=group_dict)
    #
